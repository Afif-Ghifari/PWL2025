...
class PenjualanController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'barang_id' => 'required|array', // array barang_id
            'barang_id.*' => 'required|exists:m_barang,barang_id', // isi array barang_id
        ...
        DB::beginTransaction(); // mulai transaksi ke db tapi belum disimpan
        try {
            ...
            DB::rollBack(); // batal simpan ke db
            ...
            if (count($data) > 1) { // jika data lebih dari 1 baris 
                foreach ($data as $baris => $value) {
                    if ($baris > 1) { // baris ke 1 adalah header, maka lewati 
                        ...
                        // format tanggal ke database
                        $tanggal = Date::excelToDateTimeObject($tanggalExcel)->format('Y-m-d');
                        
                        // mengambil kode penjualan
                        $tempKode = $kode;
